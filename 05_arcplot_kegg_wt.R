...
# certain signalling pathways in old and young wt mice
...
co_corr_young <- fromJSON(file = '~/Documents/Collaborations/Martinez_HPC/tables/220429_co_expression_wt_young.json')

mean_expr_young <- read.table(file = "~/Documents/Collaborations/Martinez_HPC/tables/220429_mean_expression_hep_wt_young.csv", 
                              header = TRUE, sep=','
                              )
...
pathway_groups <- pathway_groups[pathway_groups$genes %in% unique(c(edge_list_coex$V1, 
                                                                    edge_list_coex$V2)),]
#remove duplicates
...
# plot arc diagram
pdf(file = paste0('~/Documents/Collaborations/Martinez_HPC/figures/', 
                  today, '_arcplot_', grouplabel, '_young.pdf'),
    width= 11.69,
      height= 8.27
    )
...
# read result files from python.
co_corr_old <- fromJSON(file = '~/Documents/Collaborations/Martinez_HPC/tables/220429_co_expression_wt_old.json')

mean_expr_old <- read.table(file = "~/Documents/Collaborations/Martinez_HPC/tables/220429_mean_expression_hep_wt_old.csv", 
                              header = TRUE, sep=','
)
...
for (idx in 1:length(co_corr_old)){
  ...
  if (sum(tmp_id %in% mean_expr_old$X)==2){
    ...
    #keep hnf4a and cepba connections
    #  else{
    #    if (sum(tmp_id %in% c(hnf4a, cebpa))>0){
    #      edge_list_coex <- rbind(edge_list_coex, tmp_id)
    #      edge_weights_coex <- rbind(edge_weights_coex, co_corr_old[idx][names_coex])
    #    }
    #  }
...
#DEPRECATED for KEGG pathways
# pathway_groups <- matrix(nrow=0, ncol=2)
# #get group of pathways, i.e. gene - pathway id 
# for (idx in 1:length(hnf4a_pathways)){
#   
#   gene_names <- kegg[[hnf4a_pathways[[idx]]]]$GENE
#   pathway_tmp <- data.frame(group = rep(idx, length(gene_names)), genes = gene_names)
#   pathway_groups <- rbind(pathway_groups, pathway_tmp)
# }
# 
# for (idx2 in 1:length(cebpa_pathways)){
#   idx <- idx2 + 2
#   gene_names <- kegg[[cebpa_pathways[[idx2]]]]$GENE
#   pathway_tmp <- data.frame(group = rep(idx, length(gene_names)), genes = gene_names)
#   pathway_groups <- rbind(pathway_groups, pathway_tmp)
# }
...
#pathway_groups <- pathway_groups[!duplicated(pathway_groups$genes),]
#filter for genes in mean expression table
#pathway_groups <- pathway_groups[pathway_groups$genes %in% mean_expr_old$X,]
#filter further by the unique gene names from edgelist
edgelist_names <- unique(c(edge_list_coex[,1], edge_list_coex[,2]))
#pathway_groups <- pathway_groups[pathway_groups$genes %in% edgelist_names,]
mean_expr_old <- mean_expr_old[mean_expr_old$X %in% edgelist_names,]
...
degrees_ref = mean_expr_young$gene_mean_hep
# get edges value
values = unlist(edge_weights_coex)#get.edge.attribute(g_coex, "value")
...
# plot arc diagram
pdf(file = paste0('~/Documents/Collaborations/Martinez_HPC/figures/', 
                  today, '_arcplot_', grouplabel, '_old.pdf'),
    width= 11.69,
    height= 8.27
)
